...
#
#
# def launch_listener_page(user, mydb):
...
def launch_listener_page(user, mydb):
    ...
    for member in members:
        username_id_dict[member[1]] = member[0]

    button_y = 0.6
    for member in members:
        username = member[1]
        chat_button = customtkinter.CTkButton(master=frame, text=f"Chat with {username}", corner_radius=6,
                                              command=lambda username=username: initiate_chat(username))
    ...
    logout_button = customtkinter.CTkButton(master=frame, text="Logout", corner_radius=6, command=logout)
    logout_button.place(relx=0.5, rely=0.9, anchor=tkinter.CENTER)
